...
void writeCommand(unsigned char c) {
    ...
    //Enable chip select. This is needed if there are multiple slaves
    MAP_SPICSEnable(GSPI_BASE);

    //Write to DC pin -- make low to transfer over to command register
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0);

    //Write to CS Pin 62, low -- initiates MCU communication
    ...
    //Write to CS pin 62, CS Pin high
...
void writeData(unsigned char c) {
    ...
    //Write to DC pin -- make high
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0x80);

    //Write to CS pin 62 -- make low
    GPIOPinWrite(GPIOA0_BASE, 0x80, 0);

    //Send data byte c into the OLED
    SPIDataPut(GSPI_BASE, c);

    //Clean data/clear buffer
    SPIDataGet(GSPI_BASE, &dummy);

    //Write to CS pin 62, high -- stops MCU communication
    GPIOPinWrite(GPIOA0_BASE, 0x80, 0x80);
