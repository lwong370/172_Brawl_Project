...
void testMultiColorH() {
   ...
   // Loop so that we draw lines that allow us to only draw 8 lines on the screen
   for (y=0; y < height()-1; y+=16) {
     drawFastHLine(0, y, width()-1, color);

     // Increments the color hexadecimal value
     color+=100;
   }
     delay(100);
}
...
//*****************************************************************************
void testMultiColorV() {
    unsigned int x;
    int color = 0xF800;
   fillScreen(BLACK);
   for (x=0; x < width()-1; x+=16) {
       drawFastVLine(x, 0, height()-1, color);
       color+=100;
   }
     delay(100);
}
...
void printCharSet(void) {
    ...
    char *font_str;
    int ASCIIcharTotalLen = (int)(sizeof(font)/sizeof(font[0])); // Calculates the total amount of fonts in the file given

    // Clear screen
    fillScreen(BLACK);

    // Iterate over all the fonts
    for(i = 0; i < ASCIIcharTotalLen; i++) {

        // if the x position surpasses 115 units on the OLED
        if(x > 115) {
            y+=8;
            x = 0;
        }

        // if the y position surpasses 127 units on the OLED
        if(y > 127) {
            fillScreen(BLACK);
            x = 0;
            y = 0;
        ...
        // Get font character
        font_str = (char *)&font[i];

        // Draw the font character on the OLED at the current x and y positions
        drawChar(x, y, font_str, CYAN, BLACK, 1);
    }
}
